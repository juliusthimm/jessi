...
export const IndividualReports = ({
  ...
  return <Accordion type="single" collapsible className="space-y-2">
      {analyses.map(record => <AccordionItem key={record.id} value={record.id} className="px-3 py-2 rounded-lg backdrop-blur-lg border border-white/10 bg-slate-800">
          ...
          <AccordionContent>
            ...
            return <CategoryScore key={`${record.id}-${topic.id}`} title={topic.title} score={topicData?.value ?? null} description={topic.description} rationale={topicData?.rationale} />;
          })}
            </div>
          </AccordionContent>
        </AccordionItem>)}
