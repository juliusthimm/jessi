...
const Home = () => {
  ...
  useEffect(() => {
    const fetchUserRole = async () => {
      try {
        ...
        const {
          data: member
        } = await supabase.from('company_members').select('role').eq('user_id', user.id).single();
  ...
  const renderDashboard = () => {
    const commonElements = <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {!showAssessment ? <Card className="p-6 bg-white/5 backdrop-blur-lg border-white/10">
              <div className="text-center space-y-6">
                ...
                <Button size="lg" className="bg-pulse-700 hover:bg-pulse-600 text-white" onClick={() => setShowAssessment(true)}>
                  Start Assessment
                </Button>
              </div>
            </Card> : <ChatInterface onComplete={() => setShowAssessment(false)} />}

          {/* Admin Stats */}
          {userRole === 'admin' && <Card className="p-6 bg-white/5 backdrop-blur-lg border-white/10">
          ...
          {/* HR Actions */}
          {userRole === 'hr' && <Card className="p-6 bg-white/5 backdrop-blur-lg border-white/10">
  ...
  return <div className="max-w-7xl mx-auto px-4 py-8">{renderDashboard()}</div>;
};
export default Home;